...
export class WinCondition {
    ...
    to the HUD*/
    horizontalLine(symbol) { /*returns a boolean if any horizontal 
    ...
    verticalLine(symbol) {
        return this.field.some( (row, index) => {
            ...
            return (
                inspectingColumn.every( slot => slot.occupied && slot.symbol == symbol
                    ) && inspectingColumn   
            );
        });
    }
    ...
    diagonalLine(symbol) {
        ...
        if ( !this.field[middle][middle].occupied &&
        ...
        let column = this.field[0][0].occupied && this.field[0][0].symbol === symbol
            ? 0 : this.field[0][length].occupied && this.field[0][length].symbol === symbol
                ? length : false;
        ...
        return (inspectingDiagonal.every(slot => slot.occupied && slot.symbol == symbol
            ) && inspectingDiagonal);
    }
