...
export class GameHud {
    ...
    processGameEnd() {
        ...
        if(this.gameEngine.isWinner) {
            new Notice(
                `Game Ended! Winner is ${this.gameEngine.turnOf}! Game took ${
                    this.turns} turns`, 1500
            );
            winner = this.gameEngine.turnOf;
        } else if(this.gameEngine.isTie) {
            new Notice(`Game Ended! It's a tie! Game took ${this.turns} turns`,
                1500);
        }
    ...
    occupyField(element) {
        ...
        this.gameField.getItem(turnAction.row)
